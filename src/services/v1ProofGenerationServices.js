...
import { initTOK, convert } from '../helpers/TOKClient'
import { InfoError } from '../helpers/errorHelper'
import logger from '../config/logger'

const mainnetRPCLength = config.app.TOKRPC.length // total mainnet rpcs
...
export async function isBlockIncluded(blockNumber, isMainnet, version) {
  const { ethereumRPC, TOKRPC, maxRetries, rpcLength } = getVersionDetails(version)
  ...
  for (let i = 0; i < maxRetries; i++) {
    ...
    try {
      // initialize TOK client
      const rootChain = await initTOK(
        isMainnet,
        version,
        TOKRPC[rpcIndex],
        ethereumRPC[rpcIndex]
      ).then((TOKClient) => {
        return TOKClient.exitUtil.rootChain
      })
...
export async function fastMerkleProof(start, end, number, isMainnet, version) {
  ...
  for (let i = 0; i < maxRetries; i++) {
    ...
    try {
      ...
      // get merkle proof
      proof = await TOKClient.exitUtil.getBlockProof(number, { start, end })
  ...
  for (let i = 0; i < maxRetries; i++) {
    ...
    try {
      ...
      try {
        ...
        isCheckpointed = await TOKClient.exitUtil.isCheckPointed(burnTxHash)
      ...
      try {
        result = await TOKClient.exitUtil.buildPayloadForExit(
          burnTxHash,
          eventSignature,
          false,
          tokenIndex
...
export async function generateAllExitPayloads(
  ...
  for (let i = 0; i < maxRetries; i++) {
    ...
    try {
      ...
      // build payload for exit
      try {
        result = await TOKClient.exitUtil.buildMultiplePayloadsForExit(
          burnTxHash,
          eventSignature,
          false
        )
