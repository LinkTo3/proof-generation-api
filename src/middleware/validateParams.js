...
export default {
  ...
  validateV1NetworkParam: (req, res, next) => {
    ...
    try {
      // network can either be TOK or mumbai
      if (network !== 'TOK' && network !== 'mumbai' && network !== 'amoy') {
        return handleBadRequest({
          res,
          errMsg: `Invalid network ${network}. Network can either be TOK, mumbai or amoy for PoS v1 routes`
        })
      }
      next()
    } catch (error) {
